...
bool StateParser::parseState(const char *stateFile, string stateID, vector<CGameObject *> *pObjects, std::vector<std::string> *pTextureIDs)
{
...
void StateParser::parseTextures(TiXmlElement* pStateRoot, std::vector<std::string> *pTextureIDs)
{
    ...
    for(TiXmlElement* e = pStateRoot->FirstChildElement(); e != NULL; e = e->NextSiblingElement())
    {
        ...
        TheTextureManager::Instance()->load(filenameAttribute, idAttribute, CGame::Instance()->GetRenderer());
    }
}

void StateParser::parseObjects(TiXmlElement *pStateRoot, std::vector<CGameObject *> *pObjects)
{
    for(TiXmlElement* e = pStateRoot->FirstChildElement(); e != NULL; e = e->NextSiblingElement())
    {
        ...
        //int x, int y, int width, int height, std::string textureID, int numFrames, void()
         CGameObject* pGameObject = TheGameObjectFactory::Instance()->create(e->Attribute("type"));
        pGameObject->load(std::unique_ptr<CLoaderParams>(new CLoaderParams(x, y, width, height, textureID, numFrames, callbackID, animSpeed)));
