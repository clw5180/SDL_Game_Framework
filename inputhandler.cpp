...
void CInputHandler::onJoystickAxisMove(SDL_Event &event)
{
	...
	//clw note   我的北通手柄具体按键配置和飞机大战的example，以及sdlpal都不一样
	//event.jaxis.axis == 0、1、2、3，控制左右按键
    //event.jaxis.axis == 4，控制上下按键
	//而在这里，用0和3控制左右，1和4控制上下


    // left stick move left or right        
    if(event.jaxis.axis == 0)      //clw 我的北通手柄和例子不一样，event.jaxis.axis == 1实际控制左右按键
	{
	...
	/*move_axis0~3变量值为32767，即代表向右移动，而move_axis4值为-256  */
	/*按下手柄十字键的↓键时，move_axis0~3值为-256,而move_axis4值为32767*/
	/*说明北通BTP-1126型手柄x轴由axis0~3均可表示，y轴由axis4表示，很另类*/
	/*SDL官方的wiki对于SDL_JoystickGetAxis函数下面给出了一个Remarks：  */
	/*对于多数现代的摇杆，x轴通常由axis 0表示而y轴通常由axis 1表示      */
